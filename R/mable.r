...
#'  estimated by Bernstein polynomial \eqn{f_m(x; p)=\sum_{i=0}^m p_i\beta_{mi}[(x-a)/(b-a)]},
...
#'   i=0,\ldots,m\}-1} and \eqn{\epsilon} is chosen as \code{.Machine$double.eps}.
#' @return  A list with components
#' \itemize{
#'   \item \code{m} the selected/given optimal degrees by methods of change-point and BIC
#'   \item \code{p} a list of \code{p.cp} and \code{p.bic}, the estimated \code{p} with degree \code{m}
...
mable<-function(x, M, interval=c(0,1), IC=c("none", "aic", "hqic", "all"),
    ...
    if(k==0){
        ...
        ans<-list(p=res[[3]], m=m, mloglik=res[[7]], support=c(a,b), convergent=!res[[8]], del=res[[9]])
    }
    else{
        #p<-rep(1, 2*M[2]+2)
        p<-rep(1, M[2]+1)
        ...
        ans<-list(p=res[[3]][1:(m[1]+1)], #p.bic=res[[3]][(m[1]+2):(m[1]+m[2]+2)]),
            m=m, mloglik=mloglik, lk=lk, lr=res[[8]][1:k], M=M,
            support=c(a,b), pval=res[[10]][1:(k+1)], ic=ic,
...
#' @return A list with components
#' \itemize{
#'   \item \code{m} the selected/given optimal degree by the method of change-point #and BIC
#'   \item \code{p} #a list of \code{p.cp} and \code{p.bic},
...
mable.group<-function(x, breaks, M, interval=c(0, 1), IC=c("none", "aic", "hqic", "all"),
    ...
    if(k==0){
        ...
        ans<-list(p=res[[4]], m=m, mloglik=res[[8]], support=c(a,b), convergence=!res[[9]], del=res[[10]])
    }
    else{
        lk<-rep(0, k+1)
        lr<-rep(0, k)
        ...
        ans<-list(p=res[[3]][1:m[1]],
            mloglik=mloglik, m=optim, lk=lk, lr=res[[9]][1:k], M=M, support=c(a,b),
...
#        fm(x; p) or Fm(x;p)
##############################################
# Returns n-dim row vector, n=length(x)
#' Bernstein polynomials
#' @description The Bernstein polynomials at \code{x} with coefficients \code{p} on an \code{interval}
#' @param x sequence of values in (a,b) at which density/distribution function to be evaluated
#' @param p mixing proportions. The \code{m+1} components of \code{p} must be nonnegative.
#' @param interval a vector containing the endpoints of supporting/truncation interval
...
#'                expression(B^{f}*(x))))
#'
# @useDynLib mable mable_approx .registration = TRUE
#' @keywords  distribution  models  nonparametric  smooth
#' @concept Bernstein polynomial model
#' @export
bern.poly<-function(x, p, interval=c(0, 1), density=TRUE){
    m<-length(p)-1
    if(any(p<0)) stop("Negative mixture proportion.")
    n<-length(x)
    a<-interval[1]
    b<-interval[2]
    if(a>=b) stop("a must be smaller than b")
...
#    cdf<-!density
    res<-.C("mable_approx",
      as.double(u), as.double(p), as.integer(m), as.integer(n), as.logical(!density))
...
plot.mable<-function(x, which=c("density", "cumulative", "survival", "likelihood",
    ...
    if(any(which=="density")||any(which=="all")){
        yy<-bern.poly(xx, phat[1:(m+1)], c(a, b), density=TRUE)
        if(!add)
    ...
    if(any(which=="cumulative")||any(which=="survival")||any(which=="all")){
        if(any(which=="survival")||any(which=="all"))
            yy<-1-bern.poly(xx, phat[1:(m+1)], c(a, b), density=FALSE)
        else
            yy<-bern.poly(xx, phat[1:(m+1)], c(a, b), density=FALSE)
        if(!add) {
        ...
        if(any(which=="density")||any(which=="all")){
            fn<-function(x1, x2) mvbern.poly(cbind(x1, x2), x)
        ...
        if(any(which=="cumulative")||any(which=="all")){
            fn<-function(x1, x2) mvbern.poly(cbind(x1, x2), x, density=FALSE)
            x1 <- seq(a[1], b[1], length= 40)
            x2 <- seq(a[2], b[2], length= 40)
            z2 <- outer(x1, x2, fn)
...
summary.mable_reg<-function(object, ...){
    ...
    cat("\nData:", obj$data.name,"\n")
    cat("Obj Class:", cl,"\n")
    cat("Dimsion of response:", ans$dim,"\n")
    cat("Dimsion of covariate:", ans$dimcov,"\n")
    cat("Optimal degree m:", ans$m,"\n")
...
optim.gcp<-function(obj){
    ...
    ans<-list(m=res[[4]], M=M, mloglik=lk[res[[4]]-M[1]+1], lk=lk, lr=res[[3]],
        support=obj$support, pval=res[[5]], chpts=res[[6]]+M[1])
    class(ans)<-"mable"
    return(ans)
}
#########################################################
# generating prn from sum(p[i]*beta[m,i](x): i=0,...,m)
#########################################################
#' Generate data from Bernstein polynomial model
#' Generate random sample from mixture of beta distributions with
#'  shapes \eqn{(i+1, m-i+1)}, \eqn{i=0,\ldots,m}, with given mixture
#'  proportions \eqn{p=(p_0, \ldots,p_m)}.
#' @param n sample size
#' @param p a vector of mixture proportions, must contain at least four components
#' @importFrom stats rbeta
#' @export
rmixbeta<-function(n, p){
    if(length(p)==0) stop("Missing mixture proportions 'p' without default.")
    if(length(p)<=3) stop("'p' must contain at least four components.")
    m<-length(p)-1
    w<-sample(0:m, n, replace = T, prob = p)
    rbeta(n, shape1 = w+1, shape2 = m-w+1)
}
